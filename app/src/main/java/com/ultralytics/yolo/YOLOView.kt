...
class YOLOView @JvmOverloads constructor(
    ...
    // Class filtering
    // null = not initialized (show all), emptySet() = explicitly disabled (show none), non-empty = show only these
    private var enabledClasses: Set<String>? = null // null means all classes enabled (initial state)
    
    fun setEnabledClasses(classes: Set<String>) {
        enabledClasses = classes
        // Invalidate overlay to redraw with filtered classes
        post {
            overlayView.invalidate()
        }
    }
    
    fun getEnabledClasses(): Set<String> = enabledClasses ?: emptySet()
    
    fun getAvailableClasses(): List<String> {
        return predictor?.labels ?: emptyList()
    }
    ...
    private fun onFrame(imageProxy: ImageProxy) {
        ...
        if (predictor == null) {
            Log.w(TAG, "onFrame: Predictor is null, skipping inference")
            imageProxy.close()
            return
        }
        ...
        predictor?.let { p ->
            ...
            if (!shouldRunInference()) {
                ...
                Log.d(TAG, "onFrame: Inference should run, proceeding...")
            }
            
            try {
                Log.d(TAG, "onFrame: Starting inference, bitmap size: ${bitmap.width}x${bitmap.height}")
                ...
                Log.d(TAG, "onFrame: Inference completed, detected ${result.boxes.size} objects")
                ...
                // Update overlay
                post {
                    Log.d(TAG, "onFrame: Invalidating overlay, showOverlays=$showOverlays, boxes=${result.boxes.size}")
                    overlayView.invalidate()
                }
            } catch (e: Exception) {
    ...
    private inner class OverlayView(context: Context) : View(context) {
        ...
        override fun onDraw(canvas: Canvas) {
            ...
            if (!showOverlays) {
                Log.d(TAG, "OverlayView.onDraw: showOverlays is false, skipping draw")
                return
            }
            
            Log.d(TAG, "OverlayView.onDraw: Drawing ${result.boxes.size} boxes, task=${task}")
            ...
            when (task) {
                ...
                YOLOTask.DETECT -> {
                    ...
                    // Filter boxes by enabled classes
                    // null = show all, emptySet() = show none, non-empty = show only these
                    val filteredBoxes = when {
                        enabledClasses == null -> result.boxes // Not initialized, show all
                        enabledClasses!!.isEmpty() -> emptyList() // Explicitly disabled, show none
                        else -> result.boxes.filter { box -> box.cls in enabledClasses!! } // Filter by enabled classes
                    }
                    
                    Log.d(TAG, "Filtered boxes: ${filteredBoxes.size} (enabled classes: ${enabledClasses?.size ?: "all"})")
                ...
                YOLOTask.SEGMENT -> {
                    ...
                    // Segmentation mask - only show masks for filtered boxes
                    if (filteredBoxes.isNotEmpty() || enabledClasses == null) {
                        result.masks?.let { masks ->
                            // Get mask bitmap - either combined (if all classes) or filtered
                            val maskBitmap = if (enabledClasses == null || enabledClasses!!.isEmpty()) {
                                // Show all masks or no masks
                                if (enabledClasses == null) {
                                    masks.combinedMask
                                } else {
                                    null // Explicitly disabled, show no masks
                                }
                            } else {
                                // Generate filtered mask from individual masks
                                // Get mask dimensions from masks.masks if combinedMask is null
                                val maskWidth = masks.combinedMask?.width ?: (if (masks.masks.isNotEmpty() && masks.masks[0].isNotEmpty()) masks.masks[0][0].size else 0)
                                val maskHeight = masks.combinedMask?.height ?: (if (masks.masks.isNotEmpty()) masks.masks[0].size else 0)
                                generateFilteredMask(masks, result.boxes, filteredBoxes, maskWidth, maskHeight)
                            }
                            ...
                            maskBitmap?.let { bitmap ->
                                ...
                                if (isFrontCamera) {
                                    ...
                                    canvas.scale(-1f, 1f)
                                    canvas.translate(-vw / 2f, 0f)
                                    canvas.drawBitmap(bitmap, src, dst, maskPaint)
                                    canvas.restore()
                                } else {
                                    canvas.drawBitmap(bitmap, src, dst, maskPaint)
                                }
    ...
    /**
     * Generate a filtered mask bitmap containing only masks for the filtered boxes
     * @param masks The Masks object containing all masks
     * @param allBoxes All boxes from the result
     * @param filteredBoxes Only the boxes that should be shown
     * @param maskWidth Width of the mask
     * @param maskHeight Height of the mask
     * @return Filtered mask bitmap or null if no masks to show
     */
    private fun generateFilteredMask(
        masks: Masks,
        allBoxes: List<Box>,
        filteredBoxes: List<Box>,
        maskWidth: Int,
        maskHeight: Int
    ): Bitmap? {
        if (filteredBoxes.isEmpty() || masks.masks.isEmpty() || maskWidth <= 0 || maskHeight <= 0) {
            return null
        }
        
        // Find indices of filtered boxes in allBoxes
        // filteredBoxes is a subset of allBoxes, so we can match by comparing box properties
        val filteredIndices = mutableSetOf<Int>()
        for (filteredBox in filteredBoxes) {
            // Try to find matching box in allBoxes by comparing key properties
            val index = allBoxes.indexOfFirst { box ->
                box.index == filteredBox.index && 
                box.cls == filteredBox.cls && 
                kotlin.math.abs(box.conf - filteredBox.conf) < 0.001f
            }
            if (index >= 0 && index < masks.masks.size) {
                filteredIndices.add(index)
            }
        }
        
        if (filteredIndices.isEmpty()) {
            Log.w(TAG, "No matching indices found for filtered boxes")
            return null
        }
        
        // Generate combined mask from filtered indices
        val combinedPixels = IntArray(maskWidth * maskHeight) { Color.TRANSPARENT }
        val threshold = 0.5f
        
        filteredIndices.forEach { detIndex ->
            if (detIndex < masks.masks.size && detIndex < allBoxes.size) {
                val maskData = masks.masks[detIndex]
                val box = allBoxes[detIndex]
                val color = ultralyticsColors[box.index % ultralyticsColors.size]
                
                // Apply mask threshold and set pixels
                for (y in maskData.indices) {
                    if (y < maskHeight) {
                        val row = maskData[y]
                        for (x in row.indices) {
                            if (x < maskWidth) {
                                val prob = row[x]
                                if (prob > threshold) {
                                    combinedPixels[y * maskWidth + x] = color
                                }
                            }
                        }
                    }
        ...
        bmp.setPixels(combinedPixels, 0, maskWidth, 0, 0, maskWidth, maskHeight)
        return bmp
    }
    ...
    private inner class ScaleListener : ScaleGestureDetector.SimpleOnScaleGestureListener() {
        ...
        override fun onScale(detector: ScaleGestureDetector): Boolean {
            ...
            // Update zoom label
            zoomLabel.text = String.format("ZOOM: %.1fx", currentZoomRatio)
